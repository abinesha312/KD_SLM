"""Qwen2.5-Omni student model wrapper (text-only path)."""
...
class QwenStudent:
    def __init__(
        ...
        self.cfg = cfg
        self.model_id = str(model_path or cfg.student_id)
        self.adapter_path = str(adapter_path) if adapter_path else None
        self.processor = None
        self.model = None

    def download(self) -> Path:
        cache = self.cfg.resolve(self.cfg.cache_dir) / "qwen_student"
        cache.mkdir(parents=True, exist_ok=True)
        logger.info("Downloading student model %s", self.cfg.student_id)
        snapshot_download(
            repo_id=self.cfg.student_id,
            local_dir=str(cache),
            token=self.cfg.hf_token,
        )
        return cache

    def load(self, model_path: str | Path | None = None, adapter_path: str | Path | None = None) -> None:
        path = str(model_path or self.model_id)
        adapter = str(adapter_path or self.adapter_path) if (adapter_path or self.adapter_path) else None
        logger.info("Loading Qwen student from %s%s", path, f" with adapter {adapter}" if adapter else "")

        self.processor = Qwen2_5OmniProcessor.from_pretrained(path, token=self.cfg.hf_token)
